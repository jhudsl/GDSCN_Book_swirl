...
**Q4.** What time period does this dataset cover?  In other words
...
:::{.reflection}
**Q5.** What is the average case count for Hepatitis A in Maryland?

- Copy the above command, and run it in RStudio.
- Then use an R command to find the average of the `count` column in the `hepatitis_md` dataframe.

**Q6.** Modify the above command to select for Polio cases in Vermont, and find the average count.

- Remember that you can use the up arrow to cycle through previous commands and can then modify and run them.
- Tap the up arrow until you get back to the command that extracted the data for Hepatitis A in Maryland.
...
For example, the median case count for Polio in New York is 685.  That's much higher than Vermont!

But let's use the data to see whether the case counts are comparable when we adjust for the number of people in each state.
...
First, extract the New York Polio data, just like you did for the Vermont data, and store it in a new dataframe.

Then, for each of these dataframes calculate the case count per 100,000 people (this is a common way to report disease frequencies), and then find the median for each state.
